...
module.exports = grammar({
  ...
  rules: {
    ...
    pattern: (_) => seq('$"', optional(repeat(choice(/[^"]+/, '\\"'))), '"'),

    namespace: ($) => seq("namespace", $.qualified_name, ";"),
    ...
    list_expression: ($) =>
      seq(
        optional("mut"),
        "[",
        optional(seq(comma_separated($.expression), optional(","))),
        "]",
      ),
    ...
    variable_expression: ($) =>
      choice(
        prec.right(
          PREC.ASSIGNMENT,
          seq($.qualified_name, seq("=", $.expression)),
        ),
        $.qualified_name,
      ),
    ...
    if_expression: ($) =>
      seq("if", "(", $.expression, ")", $.expression, "else", $.expression),
    ...
    if_statement: ($) =>
      seq(
        ...
        $.expression,
        optional(
          choice(seq("->", $.identifier), seq("as", $.identifier, ":", $.type)),
        ),
        ")",
        $.block,
    ...
    try_statement: ($) =>
      seq(
        "try",
        $.block,
        seq(repeat($.catch_clause), optional($.catch_only_clause)),
      ),

    catch_clause: ($) =>
      seq("catch", "(", $.identifier, ":", $.type, ")", $.block),
    ...
    zdef_statement: ($) =>
      seq("zdef", "(", $.string_literal, ",", $.string_literal, ")", ";"),
    ...
    function_type: ($) =>
      prec.left(
        ...
        seq(
          "fun",
          optional(
            seq(optional($.identifier), "::<", comma_separated($.type), ">"),
          ),
          "(",
          optional(
            comma_separated(
          ...
          optional(seq("*>", $.type)),
          optional(
            seq("!>", optional("("), comma_separated($.type), optional(")")),
          ),
        ),
      ),

    fun_declaration: ($) => seq($.function_signature, $.block),
    ...
    object_field: ($) =>
      choice(
        ...
        seq(optional(choice("mut", "static")), $.arrow_fun_declaration, ";"),
      ),
        ...
        seq(
          choice(
            ...
            seq(
              ...
              optional(
                seq(
                  comma_separated(seq(optional($.identifier), ":", $.type)),
                  optional(","),
                ),
              ),
              "}",
    ...
    qualified_var_declaration: ($) =>
      choice(
        seq(choice("var", "final"), $.var_declaration),
        seq(
          "_",
          choice(seq(":", $.type, "=", $.expression), seq("=", $.expression)),
        ),
      ),
    ...
    binary_expression: ($) =>
      choice(
        ...
        prec.left(PREC.EQUALITY, seq($.expression, "!=", $.expression)),
        prec.left(PREC.NULL_COALESCING, seq($.expression, "??", $.expression)),
