...
module.exports = grammar({
  ...
  rules: {
    ...
    prefixed_identifier: $ => $.qualified_name,
    ...
    anonymous_object_init: $ =>
      seq(optional('mut'), '.', '{', optional(comma_separated($.qualified_var_declaration)), '}'),
    ...
    for_statement: $ =>
      seq(
        'for',
        '(',
        comma_separated($.var_declaration),
        ';',
    ...
    as_expression: $ =>
      prec.left(PREC.IS_AS, seq($.expression, 'as', $.type)),

    list_expression: $ =>
      seq(
        optional('mut'),
        '[',
        optional(seq('<', $.type, '>', optional(','))),
        optional(comma_separated($.expression)),
        ']'
      ),
    ...
    map_expression: $ =>
      seq(
        optional('mut'), 
        '{',
        optional(seq('<', $.type, ':', $.type, '>', optional(','))),
        optional(
          seq( 
    ...
    variable_expression: $ => $.qualified_name,
    ...
    catch_clause: $ => seq('catch', '(', $.type, $.identifier, ')', $.block),

    catch_only_clause: $ => seq('catch', $.block),

    while_statement: $ => seq('while', '(', $.expression, ')', $.block),

    zdef_statement: $ => seq('zdef', '(', $.string, ',', $.string, ')', ';'),
    ...
    object_field: $ =>
      choice(
        ...
        seq(optional('static'), $.fun_declaration),
        $.arrow_fun_declaration,
      ),
      ...
      prec.left(
        ...
        seq(
          choice(
            ...
            $.function_type,
            seq(optional('mut'), $.qualified_name),
            seq(optional('mut'), 'obj', '{', comma_separated($.qualified_var_declaration), '}'),
            seq('fib', '<', $.type, ',', $.type, '>'),
          ),
          optional('?')
        )
    ...
    call_expression: $ =>
      prec.left(PREC.CALL, seq($.expression, '(', optional($.argument_list), ')', optional(seq('catch', $.expression)))),
